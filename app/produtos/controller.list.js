angular.module('app').controller(
  ...
  ['$rootScope', '$scope', '$location', 'ProdutosResource', 'PaginationFactory', 'InputFocusFactory',

// controller definition
function ($rootScope, $scope, $location, resource, pagination, input) {

  var ctrlName = 'ProdutosListCtrl';
  ...
  */
  ...
  $rootScope.$on('produtos:add:event', function(event, value) {
    event.preventDefault(); event.stopPropagation();
    //console.log('produtos:add:event - ' + value);
    pagination.addCheck();
    updateLocation();
  });

  $rootScope.$on('produtos:update:event', function(event, value) {
    event.preventDefault(); event.stopPropagation();
    //console.log('produtos:add:event - ' + value);
    updateLocation();
  });

  $rootScope.$on('produtos:remove:event', function(event, value) {
    event.preventDefault(); event.stopPropagation();
    //console.log('produtos:remove:event - ' + value);
  ...
  $scope.setPage = function() {
    if((this.n+1) != $scope.produtos.page) {
